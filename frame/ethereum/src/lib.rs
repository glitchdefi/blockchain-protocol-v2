...
#[derive(PartialEq, Eq, Clone, Encode, Decode, MaxEncodedLen, RuntimeDebug, scale_info::TypeInfo)]
pub enum RawOrigin {
...
pub mod pallet {
		...
		+ pallet_evm::Config
	{
		...
		/// The accounts in the revenue sharing program.
...
impl<T: Config> Pallet<T> {
	...
	fn validate_transaction_common(
		...
		let is_lower_priority = match transaction_data.action {
			TransactionAction::Call(address) => {
				transaction_data.input.len() != 0 && !T::RevenueSharing::is_in_white_list(address)
			},
			TransactionAction::Create => false
		};

		let priority = priority / (if is_lower_priority { 20_000 } else { 10_000 }) - T::GasWeightMapping::gas_to_weight(gas_limit.unique_saturated_into());
