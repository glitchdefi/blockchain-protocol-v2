...
pub use fc_rpc_core::{DebugApiServer, TraceParams};
...
use fc_tracer::{formatters::ResponseFormatter, types::single};
use fp_trace_apis::{DebugRuntimeApi, TracerInput};
use fp_trace_rpc::{RequestBlockId, RequestBlockTag};
	...
	C::Api: EthereumRuntimeRPCApi<B>,
{
	...
	fn handle_block_request(
		...
		let f = || -> RpcResult<_> {
			...
			Ok(fp_trace_apis::Response::Block)
		};

		return match trace_type {
			single::TraceType::CallList => {
				let mut proxy = fc_tracer::listeners::CallList::default();
				proxy.using(f)?;
				proxy.finish_transaction();
				let response = match tracer_input {
					TracerInput::CallTracer => fc_tracer::formatters::CallTracer::format(proxy)
		...
		if let Some(block) = reference_block {
			...
			if let Some(transaction) = transactions.get(index) {
				let f = || -> RpcResult<_> {
					...
					Ok(fp_trace_apis::Response::Single)
				};
				...
				return match trace_type {
					single::TraceType::Raw {
						...
						let mut proxy = fc_tracer::listeners::Raw::new(
							disable_storage,
							disable_memory,
							disable_stack,
						);
						proxy.using(f)?;
						Ok(Response::Single(
							fc_tracer::formatters::Raw::format(proxy).unwrap(),
						))
					}
					single::TraceType::CallList => {
						let mut proxy = fc_tracer::listeners::CallList::default();
						...
						let response = match tracer_input {
							TracerInput::Blockscout => {
								fc_tracer::formatters::Blockscout::format(proxy)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e)))
							}
							TracerInput::CallTracer => {
								let mut res = fc_tracer::formatters::CallTracer::format(proxy)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e)))?;
								Ok(res.pop().unwrap())
							}
