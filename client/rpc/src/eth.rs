...
use fc_rpc_core::{
	types::{
		Block, BlockNumber, BlockTransactions, Bytes, CallRequest, FeeHistory, FeeHistoryCache, FeeHistoryCacheLimit,
...
pub struct Eth<B: BlockT, C, P, CT, BE, H: ExHashT, A: ChainApi> {
	...
	fee_history_limit: FeeHistoryCacheLimit,
	...
	async fn estimate_gas(
		...
		if let Some(from) = request.from {
			...
			if gas_price > U256::zero() {
				let balance = client.runtime_api()
		...
		let executable = move |request, gas_limit, api_version, api: sp_api::ApiRef<'_, C::Api>| -> Result<ExecutableResult> {
			let CallRequest {
				from,
				to,
				gas,
...
pub struct Net<B: BlockT, BE, C, H: ExHashT> {
	client: Arc<C>,
	network: Arc<NetworkService<B, H>>,
...
impl<B: BlockT, BE, C, H: ExHashT> Net<B, BE, C, H> {
	pub fn new(
		client: Arc<C>,
		network: Arc<NetworkService<B, H>>,
		peer_count_as_hex: bool,
...
impl<B: BlockT, BE, C, H: ExHashT> NetApiServer for Net<B, BE, C, H>
where
	C: ProvideRuntimeApi<B> + StorageProvider<B, BE>,
