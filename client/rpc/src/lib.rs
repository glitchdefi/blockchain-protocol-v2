...
pub use eth::{
	Eth, EthApiServer, EthBlockDataCache, EthFilter, EthFilterApiServer, EthTask, Net, NetApiServer,
	Web3, Web3ApiServer,
};
pub use eth_pubsub::{EthPubSubApiServer, EthPubSub};
...
pub fn error_on_execution_failure(reason: &ExitReason, data: &[u8]) -> Result<(), jsonrpsee::core::Error> {
	match reason {
		...
		ExitReason::Error(e) => {
			...
			Err(internal_err_with_data(format!("evm error: {:?}", e),
				&[]))
		}
		ExitReason::Revert(_) => {
			...
			Err(internal_err_with_data(message, &[]))
		}
		ExitReason::Fatal(e) => Err(
			internal_err_with_data(format!("evm fatal: {:?}", e), &[]))
	}
}
