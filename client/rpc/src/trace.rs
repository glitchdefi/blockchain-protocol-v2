...
pub use fc_rpc_core::{FilterRequest, TraceApiServer};

// --- std ---
use std::{collections::BTreeMap, future::Future, marker::PhantomData, sync::Arc, time::Duration};
// --- crates.io ---
use ethereum_types::H256;
use crate::{frontier_backend_client, OverrideHandle};
...
use fp_trace_apis::DebugRuntimeApi;
...
#[async_trait]
impl<B, C> TraceApiServer for Trace<B, C>
where
	B: BlockT<Hash = H256> + Send + Sync + 'static,
	B::Header: HeaderT<Number = u32>,
	C: HeaderMetadata<B, Error = BlockChainError> + HeaderBackend<B>,
	...
	C::Api: EthereumRuntimeRPCApi<B>,
{
	...
	#[instrument(skip(client, backend, _overrides))]
	fn cache_block(
		client: Arc<C>,
		backend: Arc<BE>,
		substrate_hash: H256,
		_overrides: Arc<OverrideHandle<B>>,
	) -> TxsTraceRes {
		...
		proxy.using(f)?;
		let mut traces: Vec<_> = fc_tracer::formatters::TraceFilter::format(proxy).unwrap();
		// Fill missing data.
